...
-- Landing-page experiment tables are isolated from inbound call leads and events.
-- The additive production migration is supabase/migrations/20260719051431_landing_page_experiments.sql.
...
create index if not exists landing_page_events_created_at_idx on public.landing_page_events (created_at desc);
create index if not exists landing_page_events_variant_name_idx on public.landing_page_events (variant, event_name, created_at desc);
create index if not exists landing_page_events_session_idx on public.landing_page_events (session_id, created_at desc);
create unique index if not exists landing_page_events_submission_event_unique
  on public.landing_page_events (event_name, submission_id) where submission_id is not null;
create index if not exists landing_page_leads_created_at_idx on public.landing_page_leads (created_at desc);
create index if not exists landing_page_leads_variant_idx on public.landing_page_leads (variant, created_at desc);
create index if not exists landing_page_leads_source_idx on public.landing_page_leads (source, utm_campaign, created_at desc);
