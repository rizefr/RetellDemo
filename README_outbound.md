...
- For a received invoice, Paul asks whether the caller needs the secure payment link. If the caller declines the link, Paul asks by what date payment should be expected. The signed `schedule_followup` tool resolves the caller's exact date phrase from trusted call time/timezone, persists `expected_payment_date`, logs the event, and schedules follow-up without changing the invoice's payment status. Declining a link is not treated as refusing to pay.
