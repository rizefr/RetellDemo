...
- For a received invoice, Paul asks whether the caller needs the secure payment link. A bare yes is followed by “Would you prefer text or email?” rather than inferring a delivery method from the address on file. If the caller declines the link, Paul asks by what date payment should be expected. Every supplied date phrase, including vague answers such as “soon,” goes to the signed `schedule_followup` resolver. It either requests a specific date without writing or persists the resolved `expected_payment_date`, logs the event, and schedules follow-up without changing invoice payment status. Declining a link is not treated as refusing to pay.
