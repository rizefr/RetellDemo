...
- Keep the received-invoice branch explicit: ask whether the caller needs the payment link; if not, ask by what date payment should be expected. Persist the backend-resolved expected date through `schedule_followup` without changing invoice status. Treat an explicit refusal to pay as a separate branch that asks one reason.
