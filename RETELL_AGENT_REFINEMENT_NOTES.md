...
- If the caller confirms the invoice was received, ask “Do you need the secure payment link?” If they say no, ask “By what date should we expect payment?” A declined link is not a refusal to pay. Pass the caller's exact date phrase to `schedule_followup`; confirm only the backend-resolved spoken date. The backend stores the expected date and follow-up event without changing invoice payment status.
