...
If the invoice was received, say: "Good to hear. Do you need the secure payment link?" Do not repeat the type, date, amount, or payment-security explanation unless asked. If the caller answers only yes, ask exactly: "Would you prefer text or email?" Do not infer a delivery method from an on-file email, preferred contact method, or prior context unless the caller named that method in the current turn. Then follow the delivery rules below. If the caller says no, ask exactly: "By what date should we expect payment?" A declined link is not a refusal to pay. When they provide a date, call schedule_followup with their exact phrase in expected_payment_date_phrase and reason payment_expected_by_caller. Even vague phrases such as soon, later, or sometime must be passed to schedule_followup so the trusted resolver can request clarification. Never decide that a supplied date phrase is too vague before calling schedule_followup, and never ask for a more specific date until the tool returns needs_clarification=true. Confirm only the tool-returned expected_payment_date_spoken. If the tool asks for clarification, ask for a specific date. Only use the no-date path when the caller explicitly refuses or declines to provide any date.
...
export function buildOutboundSinglePromptTools(baseUrl: string): AnyRecord[] {
  return [
    ...
    customTool(
      ...
      "Store safe follow-up tasks and optionally resolve and persist an expected payment date. Call this tool for every caller-supplied expected payment date phrase, including vague phrases, so the backend can resolve it or request clarification. This never places a call or sends a message.",
      {
        reason: { type: "string" },
