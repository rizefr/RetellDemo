...
const jobFields={job_id:z.uuid().optional(),claim_token:z.uuid().optional()};
const hasJobPair=(input:{job_id?:string;claim_token?:string})=>Boolean(input.job_id)===Boolean(input.claim_token);
...
outboundIntegrationApiRouter.post('/quickbooks/preview',async(req,res)=>{try{const input=business.extend(jobFields).strict().refine(hasJobPair).parse(req.body);res.json(await previewQuickBooksSync(input.business_id,input.job_id?{job_id:input.job_id,claim_token:input.claim_token!}:undefined));}catch(error){fail(res,error);}});
outboundIntegrationApiRouter.post('/quickbooks/apply',async(req,res)=>{try{const input=business.extend({...jobFields,preview_id:z.uuid(),preview_hash:z.string().regex(/^[a-f0-9]{64}$/)}).strict().refine(hasJobPair).parse(req.body);res.json(await applyQuickBooksPreview(input.business_id,input.preview_id,input.preview_hash,input.job_id?{job_id:input.job_id,claim_token:input.claim_token!}:undefined));}catch(error){fail(res,error);}});
