...
export function renderOutboundEmailTemplate(content: EmailTemplateContent, v: EmailInvoiceVariables) {
  ...
  const html = `<!doctype html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${h(subject)}</title></head><body style="margin:0;background:#f2f5f3;color:#302c2c;font-family:Montserrat,Arial,sans-serif"><div style="display:none;max-height:0;overflow:hidden;mso-hide:all">${h(preheader)}</div><table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr><td align="center" style="padding:28px 12px"><table role="presentation" width="600" cellpadding="0" cellspacing="0" style="width:100%;max-width:600px;background:#ffffff;border:1px solid #dbe5de;border-radius:12px"><tr><td style="padding:30px 28px;border-bottom:4px solid #108650">${brand}</td></tr><tr><td style="padding:28px">${v.demo ? '<p style="background:#fff4db;padding:12px;color:#5e4509;font-size:12px">CONTROLLED DEMO · No payment is requested for demo records.</p>' : ''}<p style="font-size:12px;letter-spacing:1px;color:#16653f;font-weight:bold">INVOICE FOLLOW-UP</p><h1 style="font-size:27px;line-height:1.25;margin:12px 0 24px">Your invoice, in one place.</h1><p style="font-size:15px;line-height:1.7">Hello ${h(v.customer_name || "there")},</p><p style="font-size:15px;line-height:1.7">${h(introduction)}</p><table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:24px 0;background:#f3f7f4;border-radius:8px">${rows.map(([label, value])=>`<tr><td style="padding:10px 14px;font-size:12px;color:#526259;width:38%;vertical-align:top">${h(label)}</td><td style="padding:10px 14px;font-size:14px;word-break:break-word">${h(value)}</td></tr>`).join("")}<tr><td colspan="2" style="padding:20px 14px;border-top:1px solid #dbe5de"><span style="font-size:12px;color:#526259">OUTSTANDING BALANCE</span><br><strong style="font-size:30px;line-height:1.5">${h(v.balance)}</strong> <span style="font-size:12px">${h(v.currency)}</span></td></tr></table>${paymentUrl ? `<table role="presentation" cellpadding="0" cellspacing="0"><tr><td style="background:#108650;border-radius:6px"><a href="${h(paymentUrl)}" style="display:inline-block;padding:16px 24px;color:white;text-decoration:none;font-size:15px;font-weight:bold">Review invoice &amp; pay securely</a></td></tr></table>` : '<p style="padding:14px;border:1px solid #dbe5de;font-size:14px;line-height:1.6">A secure payment link is not available. Our team will provide the verified payment details.</p>'}<p style="font-size:15px;line-height:1.7;margin-top:24px">${h(closing)}</p><p style="font-size:15px;line-height:1.7">${h(v.business_name)}<br>${h(contact)}</p></td></tr><tr><td style="padding:20px 28px;background:#f3f7f4;color:#526259;font-size:11px;line-height:1.7">Payment details are entered only on the secure payment page, never by phone.${v.pinnacle_brand ? '<br>2778 W 15th Street · Brooklyn, NY 11224<br><a href="https://pinnacleelevatorsolutions.com/" style="color:#16653f">pinnacleelevatorsolutions.com</a>' : ''}</td></tr></table></td></tr></table></body></html>`;
