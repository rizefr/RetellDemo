...
describe('read-only QuickBooks integration boundaries',()=>{
 ...
 it('reads complete invoices and resolves customers only by source IDs',async()=>{const execute=vi.fn().mockResolvedValueOnce({QueryResponse:{Invoice:[raw]}}).mockResolvedValueOnce({QueryResponse:{}}).mockResolvedValueOnce({QueryResponse:{Customer:[customer]}});const p=await readQuickBooksInvoices(execute,connection,now);expect(p.counts.fetched).toBe(1);expect(execute.mock.calls[0][1].query).not.toContain('Balance >');expect(execute.mock.calls[2][1].query).toContain("Id IN ('42')");});
