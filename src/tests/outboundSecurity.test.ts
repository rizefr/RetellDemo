...
describe("outbound flow guardrails", () => {
  it("uses a tool-capable subagent with required safety rules without phone binding operations", () => {
    ...
    expect(serialized).toContain("my name is Paul from {{business_name}}");
    expect(serialized).toContain("this is Paul from {{business_name}}");
    expect(serialized).toContain("I'm a virtual assistant helping {{business_name}} follow up on service accounts");
